...
public class ClientKernel {
    ...
    private FileTransferClient ftc;  //客户端发送文件类
    ...
    public void sendMessage(String str) {
        if(!dropMe) {
        	System.out.println("this is form ck.sendmessage:"+str);
            if(str.charAt(0) == '/')
    ...
    public boolean sendFile(String directory) {
    	if(!dropMe)
            try {
            	ftc.sendFile(directory);
 //           	return true;
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        try {
            ...
            while(running) {
                while(msgList.size()>0) {
                    ...
                    for(int i=0;i<data.length;i++) dataOut.write((int)data[i]);//发送请求
